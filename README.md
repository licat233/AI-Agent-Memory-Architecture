...
Create missing Vault folders, copy missing core documents and the Frontmatter Standard, preserve existing files, write an installation log, and run the validation checklist.
```
...
Update active Core, Indexes, runtime adaptation guides, routers, governance files, and the Frontmatter Standard.
...
| [PAMA Personal Execution Workflow](personal/Personal_Execution_Workflow.md) | Optional low-authority task execution templates for complex personal work |
| [Agent Personal Execution Prompt](personal/Agent_Personal_Execution_Prompt.md) | Copy-ready prompt snippets for configuring agents to use PAMA execution templates |
...
请创建缺失的 Vault 目录、复制缺失的核心架构文档和 Frontmatter 规范、保留已有文件、写入安装日志，并执行验证清单。
```
...
目标架构：AI Agent Memory Architecture v1.3.0。
...
请更新活跃的 Core、Indexes、运行时适配指南、路由器、治理文件和 Frontmatter 规范。
...
| [PAMA 个人执行工作流](personal/Personal_Execution_Workflow.md) | 复杂个人任务、目标、复盘工作的可选低权执行模板 |
| [PAMA Agent 执行提示词](personal/Agent_Personal_Execution_Prompt.md) | 配置 agent 使用 PAMA 执行模板的可复制提示词 |
