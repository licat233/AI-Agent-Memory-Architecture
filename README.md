...
Runtime note: This project has no dependency on UI-side Obsidian executor plugins. Trusted agents such as Hermes can operate the Vault directly when they follow the routers, permission rules, retrieval policy, and proposal workflow.

### Installation

Copy this prompt to your AI agent. It will install the memory architecture into your chosen Obsidian Vault or Markdown directory:
...
First ask whether I want personal/PAMA, enterprise/ARMOR, or both, and ask for the target Vault path.
Do not install any Obsidian UI plugin.
Do not use runtime memory as long-term memory.
Create the required Vault folders, copy the core architecture documents, preserve existing files, write an installation log, and run the validation checklist.
```
...
External agents with direct file tools may operate the Vault directly, as long as they follow the architecture's routers, permission rules, retrieval policy, and proposal workflow.
...
运行时说明：本项目不依赖 Obsidian UI 侧执行插件。Hermes 等受信任智能体只要遵循路由器、权限规则、检索策略和提案工作流，即可直接操作 Vault。

### 安装教程

复制下面这段内容给你的 AI Agent，它会把当前 AI 记忆架构安装到你指定的 Obsidian Vault 或 Markdown 目录：
...
先询问我要安装 personal/PAMA、enterprise/ARMOR，还是 both，并询问目标 Vault 路径。
不要安装任何 Obsidian UI 插件。
不要把 runtime memory 当作长期记忆。
请创建必要的 Vault 目录、复制核心架构文档、保留已有文件、写入安装日志，并执行验证清单。
```
...
拥有直接文件工具的外部智能体可以直接操作 Vault，前提是遵循架构的路由器、权限规则、检索策略和提案工作流。
